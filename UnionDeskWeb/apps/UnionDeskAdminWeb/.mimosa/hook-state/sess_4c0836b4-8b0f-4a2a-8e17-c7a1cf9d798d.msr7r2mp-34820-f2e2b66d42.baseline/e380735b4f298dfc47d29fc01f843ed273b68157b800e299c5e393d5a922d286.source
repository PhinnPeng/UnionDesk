...
export default function LayoutTabbar() {
	...
	useEffect(() => {
		if (isRefresh) {
			const timer = setTimeout(() => {
				setIsRefresh(appScope, false);
			}, 500);

			return () => clearTimeout(timer);
		}
	}, [isRefresh, setIsRefresh, appScope]);
	...
	}, [activeKey, openTabs, location, navigate]);
	...
	useEffect(() => {
		// 检查默认 Tab 是否缺失
		const isDefaultTabMissing = !Array.from(openTabs.keys()).includes(scopeHomePath);

		if (isDefaultTabMissing) {
			const routeTitle = flatRouteList[scopeHomePath]?.handle?.title as string;
			insertBeforeTab(appScope, scopeHomePath, {
				key: scopeHomePath,
	...
	}, [openTabs, insertBeforeTab, flatRouteList, appScope, scopeHomePath]);
	...
	useEffect(() => {
		...
		if (normalizedPath !== activeKey) {
			setActiveKey(appScope, normalizedPath);

			const routeTitle = currentRoute.handle?.title as string;

			addTab(appScope, normalizedPath, {
				key: normalizedPath,
				// 保证 label 为 string 类型，存储到 sessionStorage。
				label: isValidElement(routeTitle) ? (routeTitle as ReactElement<{ children: string }>)?.props?.children : routeTitle,
				historyState: { search: location.search, hash: location.hash },
				/* 登录之后跳转的默认路由，不可以关闭和拖拽 */
				closable: normalizedPath !== scopeHomePath,
				draggable: normalizedPath !== scopeHomePath,
			});
		}
	}, [location, currentRoute, setActiveKey, addTab, appScope, scopeHomePath]);
