...
import { DomainAddPlatformTicketTypesModal } from "./domain-add-platform-ticket-types-modal";
...
export function DomainTicketTypesPanel({ domainId }: DomainTicketTypesPanelProps) {
	...
	const openConfigTab = useCallback((ticketType: DomainTicketType, tab: DomainTicketTypeConfigTab = "attributes") => {
		const path = `/platform/domains/ticket-type-config/${encodeURIComponent(domainId)}/${encodeURIComponent(ticketType.id)}?tab=${tab}`;
		openAppScopeTab(appScopes.platform, navigate, path, {
			key: `/platform/domains/ticket-type-config/${encodeURIComponent(domainId)}/${encodeURIComponent(ticketType.id)}`,
			label: "事项类型配置",
