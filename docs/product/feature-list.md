...
> **目标态结构视图**（2026-08-12 重构，替代现状版）：以「页面 → 功能 → 操作」层级组织，反映产品**最终设计结构**而非当前实现——所有功能挂在其设计归属页面下（含未实现功能）；页面未建/待迁移的目标态页面显式标注「目标态新增·页面未建」。源数据 = §2（状态/优先级）、§3/§4（功能明细）、§7（操作矩阵）、§8（权限码表）与 prd.md §4.2/§4.3。**状态与实现以 §2 为准，维护只改源表**；操作、权限码仅引用行号，不重抄内容。
> **状态标记**（与 §1.2 术语对照）：无标记 = 已实现；`🚧` = 部分；`📅` = 规划中；`🗑` = 占位（模板遗留）；「目标态新增·页面未建」= 页面当前不存在（含待迁移）。
> **归属规则**：页面 ↔ 功能多对多，功能仅在其「主承载页」下展开（含操作/权限引用）；其余页面作引用条目（标「→ 主承载」）。
> **目标态决策**（2026-08-12 用户确认）：F4.2 模板中心沿用现状页（`/platform/ticket-config/templates`，语义待对齐）；F3.2 SLA 域端新建独立页；F4.18 并入用户管理页；F3.15/F3.16 目标态移除（业务由 F3.3 覆盖）；F4.21 与 F4.8 合并；F4.4 安全告警中心独立页。
...
- 我的工单（列表/详情）`/tickets`、`/tickets/:ticketId`
  - F1.4 我的工单/咨询历史 🚧（目标态补「咨询历史」入口）· §7 F1.4 行 · §8 F1.4 行（无码）
  - F1.5（引用：工单详情评价辅入口「确认解决后评价」→ 主承载：通知中心）
- 在线咨询窗口 `/chat`
  - F1.2 在线咨询 📅（E5 未排期，目标态成品页；现状占位「即将开放」）· §7 F1.2 行 · §8 F1.2 行（无码）
- 通知中心 `/inbox`
  - F1.10 站内信/通知中心 · §7 F1.10 行 · §8 F1.10 行（无码）
  - F1.5 满意度评价 📅（P1；**目标态主入口 = 通知中心**，现状全站无入口）· §7 F1.5 行 · §8 F1.5 行（无码）
- 个人中心 `/me`
...
- 工单队列 `目标态新增·页面未建（待迁移）`（建议 `/ticket-queue`，待 US-S3-04 落地）
  - F2.1 工单队列与详情处理 🚧（待迁移：现状由 `/platform/ticket-pool` P0 演示页临时承载）· §7 F2.1 行 · §8 F2.1 行（ticket.* 11 码）
  - F2.2 SLA 感知与高亮 📅（E4；队列/详情页内增强，无独立页）· §7 F2.2 行 · §8 F2.2 行（domain.sla.*）
- 工单详情 `目标态新增·页面未建（待迁移）`（建议 `/ticket-queue/:ticketId`）
  - F2.1（引用：详情处理面板 → 主承载：工单队列）
  - F2.2（引用：详情 SLA 高亮 → 主承载：工单队列）
- 在线咨询工作台 `目标态新增·页面未建`（建议 `/consult-workbench`，E5）
  - F2.3 在线咨询工作台 📅（E5 未排期）· §7 F2.3 行 · §8 F2.3 行（无码）
...
- 域成员管理 `/domain/settings/members`
  - F3.3 成员管理（域成员部分）· §7 F3.3 行 · §8 F3.3 行（domain.member.*）
- 域客户管理 `/domain/customers/list`
  - F3.3（引用：域客户部分 → 主承载：域客户管理页）
  - F3.5 域客户管理增强 · §7 F3.5 行 · §8 F3.5 行（domain.customer.*）
- 域角色管理 `/domain/settings/roles`
  - F3.3（引用：域角色部分 → 主承载：系统角色管理页）
- 系统角色管理（域端）`/system/role`
  - F3.13 系统角色管理（域端）· §7 F3.13 行 · §8 F3.13 行（domain.role.*）
  - （目标态：模板下发角色实例展示「模板来源 + 锁定字段」，锁定字段只读、非锁定可微调）
- 系统菜单管理（域端）`/system/menu`
...
- 域通知配置 `/domain/settings/notifications`
  - F3.11 域通知配置 📅（占位→目标态成品页）· §7 F3.11 行 · §8 F3.11 行（domain.notification_template.*）
  - F3.2（引用：通知模板部分 → 主承载：域 SLA 规则页 SLA 部分）
- 域 SLA 规则 `目标态新增·页面未建`（建议 `/domain/settings/sla`）
  - F3.2 SLA 规则与通知模板 🚧（SLA 部分主承载；现状复用平台 `/platform/sla-management`）· §7 F3.2 行 · §8 F3.2 行（domain.sla.*）
...
- （目标态移除：F3.15 系统用户管理、F3.16 系统部门管理——模板遗留占位，域端业务由 F3.3 成员管理覆盖，2026-08-12 决策）
...
- 用户管理 `/platform/user`
  ...
  - F4.18 用户导入导出 📅（**目标态并入本页工具栏操作**；现状独立占位页 `/platform/import-export`）· §7 F4.18 行 · §8 F4.18 行（platform.user.import / platform.user.offboard_pool.export）
  - （目标态：新增跨域批量停用入口——选员工→选域集→step-up 二次认证→部分成功摘要，TR-04）
...
- 组织/部门管理 `/platform/dept`
  ...
  - F4.21 组织配置 📅（**目标态并入本页**；现状独立占位页 `/platform/org-config`）· §7 F4.21 行 · §8 F4.21 行（platform.organization.*）
- 角色管理 `/platform/role`
  - F4.9 角色管理 · §7 F4.9 行 · §8 F4.9 行（platform.role.* / platform.role_permission.* / platform.role.bind）
  - （目标态：本页新增「模板」Tab——role_template 创建/apply 多域/同步/漂移状态列）
- 权限管理入口 `/platform/permission`
...
- 事项配置 `/platform/ticket-config/*`
  ...
  - F4.2 模板中心 🚧（**目标态沿用本页 templates 子页**；「域模板提炼」语义与实现待对齐；菜单已隐藏 V20260726092200）· §7 F4.2 行 · §8 F4.2 行（platform.ticket_config.template.*）
- SLA 管理 `/platform/sla-management`
  - F4.15 SLA 规则与工作日历 · §7 F4.15 行 · §8 F4.15 行（domain.sla.*，平台侧复用）
  - F3.2（引用：平台级 SLA 保留；域级 SLA 目标态迁 `/domain/settings/sla` → 主承载：域 SLA 规则页）
- 审计/登录日志统一页 `/platform/audit-logs`
  - F4.12 审计日志/登录日志（平台统一页）· §7 F4.12 行 · §8 F4.12 行（platform.log.audit.read / platform.log.login.read）
  - （引用：独立页 `/platform/log/operation-log`、`/platform/log/login-log` 双入口，目标态收敛至统一页）
...
- 系统设置 `/platform/system-settings`
  ...
  - （目标态：安全告警中心独立页，见下）
- 安全告警中心 `目标态新增·页面未建`（建议 `/platform/security-alerts`）
  - F4.4（引用：安全告警部分 → 主承载：系统设置页）
...
- 模板遗留页（占位）`/access/*`、`/route-nest/*`、`/about`、`/outside/*` 等
  - F4.22 模板遗留页面 🗑（目标态：异常页/公开页保留，demo 页移除）· §7 F4.22 行 · §8 F4.22 行（无码）

### 9.2 结构表（目标态，页面级详细说明）

> 列说明：操作/权限码均为 §7/§8 行号引用，内容以源表为准；「承载功能明细」按 F 编号分别标注状态（多 F 页面逐条列出）；页面状态：已建 / 目标态新增·页面未建 / 已建占位。

#### 9.2.1 客户端（CustomerWeb，M1）

| 序号 | 页面/模块 | 路由 | 页面状态 | 承载功能明细（名称【状态·优先级】） | 操作（§7 引用） | 权限码（§8 引用） | 备注/关键边界 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | 登录页 | `/login`、`/d/:domainCode/login` | 已建 | F1.6 登录【已实现·P0】 | §7 F1.6 行 | 无码（认证豁免） | 滑块验证 + 记住账号 + 专属域入口；忘记密码为占位 toast |
| 2 | 注册页 | `/register`、`/d/:domainCode/register` | 已建 | F1.7 注册与入域【部分·P0】 | §7 F1.7 行 | 无码（注册公开） | 本地 mock，未调真实注册 API（US-S3-02） |
| 3 | 业务域选择页 | `/domains` | 已建 | F1.8 业务域选择与切换【已实现·P0】；F1.7（引用） | §7 F1.8 行；§7 F1.7 行 | 无码 | 邀请码加入为本地 mock（归 F1.7） |
| 4 | 服务首页 | `/home` | 已建 | F1.9 服务首页【已实现·P0】 | §7 F1.9 行 | 无码 | 首页无独立 Story |
| 5 | 提单页 | `/tickets/new` | 已建 | F1.1 提交工单（动态表单）【部分·P0】；F1.3 反馈/建议【已实现·P0】 | §7 F1.1 行；§7 F1.3 行 | 无码（客户会话） | 动态表单字段未渲染（仅 title/description）；attachmentIds 恒空数组 |
| 6 | 我的工单（列表/详情） | `/tickets`、`/tickets/:ticketId` | 已建 | F1.4 我的工单/咨询历史【部分·P0】；F1.5（引用：评价辅入口） | §7 F1.4 行；§7 F1.5 行 | 无码 | 目标态补「咨询历史」入口；咨询历史/附件展示现状缺失 |
| 7 | 在线咨询窗口 | `/chat` | 已建（占位→目标态成品） | F1.2 在线咨询【规划中（E5）·P0】 | §7 F1.2 行 | 无码 | 现状占位「即将开放」 |
| 8 | 通知中心 | `/inbox` | 已建 | F1.10 站内信/通知中心【已实现·P0】；**F1.5 满意度评价【规划中（P1）·P0】主入口** | §7 F1.10 行；§7 F1.5 行 | 无码 | F1.5 目标态主入口 = 通知中心；现状全站无入口 |
| 9 | 个人中心 | `/me` | 已建 | F1.11 个人中心【部分·P0】 | §7 F1.11 行 | 无码 | 通知偏好为占位 toast |
| 10 | 修改密码页 | `/change-password` | 已建 | F1.12 修改密码【已实现·P0】 | §7 F1.12 行 | 无码 | RequireSession 守卫，强制改密不可跳过 |

#### 9.2.2 管理端-业务域端（M2 员工端 + M3 域管理后台）

| 序号 | 页面/模块 | 路由 | 页面状态 | 承载功能明细（名称【状态·优先级】） | 操作（§7 引用） | 权限码（§8 引用） | 备注/关键边界 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 11 | 域端首页/工作台 | `/home` | 已建 | F2.4 业务域端首页/工作台【已实现·P0】 | §7 F2.4 行 | §8 F2.4 行（domain.home.read） | 按权限过滤快捷入口 |
| 12 | 工单队列 | `/ticket-queue`（建议，待 US-S3-04） | **目标态新增·页面未建（待迁移）** | F2.1 工单队列与详情处理【部分·P0】；F2.2 SLA 感知与高亮【规划中（E4）·P0】（页内增强） | §7 F2.1 行；§7 F2.2 行 | §8 F2.1 行（ticket.* 11 码）；§8 F2.2 行（domain.sla.*） | 现状由 `/platform/ticket-pool` P0 演示页临时承载 |
| 13 | 工单详情 | `/ticket-queue/:ticketId`（建议） | **目标态新增·页面未建（待迁移）** | F2.1（引用）；F2.2（引用） | §7 F2.1 行；§7 F2.2 行 | §8 F2.1 行；§8 F2.2 行 | 现状 `/platform/ticket-detail` 演示面板 |
| 14 | 在线咨询工作台 | `/consult-workbench`（建议，E5） | **目标态新增·页面未建** | F2.3 在线咨询工作台【规划中（E5）·P0】 | §7 F2.3 行 | 无码 | E5 未拆 Story |
| 15 | 域事项配置 | `/domain/ticket-config` | 已建 | F3.1 工单类型设计【已实现·P0】；F3.4 事项属性与状态配置【已实现·P0】 | §7 F3.1 行；§7 F3.4 行 | §8 F3.1 行（domain.ticket_type.*）；§8 F3.4 行（domain.ticket_attribute/status.*） | 域详情内同源（平台端引用） |
| 16 | 域成员管理 | `/domain/settings/members` | 已建 | F3.3 成员/客户/角色管理（成员部分）【已实现·P0】 | §7 F3.3 行 | §8 F3.3 行（domain.member.*） | 最后管理员保护规则 |
| 17 | 域客户管理 | `/domain/customers/list` | 已建 | F3.3（客户部分）；F3.5 域客户管理增强【已实现·P0】 | §7 F3.3 行；§7 F3.5 行 | §8 F3.5 行（domain.customer.*） | 资料编辑/重置密码仅域端可写 |
| 18 | 域角色管理 | `/domain/settings/roles` | 已建 | F3.3（角色部分） | §7 F3.3 行 | §8 F3.3 行（domain.role.*） | 与系统角色管理页同源 |
| 19 | 系统角色管理（域端） | `/system/role` | 已建 | F3.13 系统角色管理（域端）【已实现·P0】 | §7 F3.13 行 | §8 F3.13 行（domain.role.*） | 目标态：模板来源 + 锁定字段只读、非锁定可微调 |
| 20 | 系统菜单管理（域端） | `/system/menu` | 已建 | F3.14 系统菜单管理（域端）【已实现·P0】 | §7 F3.14 行 | §8 F3.14 行（domain.menu.*） | scope=business 筛选 |
| 21 | 入域配置 | `/domain/onboarding`、`/domain/settings/onboarding` | 已建 | F3.6 入域配置（域端）【已实现·P0】 | §7 F3.6 行 | §8 F3.6 行（domain.invitation_code.*） | 邀请码 + 入域双开关 |
| 22 | 域基础设置 | `/domain/settings/basic` | 已建 | F3.7 域基础设置【已实现·P0】 | §7 F3.7 行 | §8 F3.7 行（domain.general.*） | 域名称/LOGO/描述 |
| 23 | 域参数配置 | `/domain/settings/config` | 已建 | F3.8 域参数配置（KV）【已实现·P0】 | §7 F3.8 行 | §8 F3.8 行（domain.config.*） | — |
| 24 | 域屏蔽词库 | `/domain/settings/blockwords` | 已建 | F3.9 域屏蔽词库【已实现·P0】 | §7 F3.9 行 | §8 F3.9 行（domain.blocked_word.*） | 去首尾空格、禁空词 |
| 25 | 域通知配置 | `/domain/settings/notifications` | 已建（占位→目标态成品） | F3.11 域通知配置【规划中（占位）·P1】；F3.2（引用：通知模板） | §7 F3.11 行；§7 F3.2 行 | §8 F3.11 行（domain.notification_template.*）；§8 F3.2 行 | 菜单与权限已就绪；F3.2 与 F3.11 语义重叠待澄清 |
| 26 | 域 SLA 规则 | `/domain/settings/sla`（建议） | **目标态新增·页面未建** | F3.2 SLA 规则与通知模板（SLA 部分）【部分·P0】 | §7 F3.2 行 | §8 F3.2 行（domain.sla.*） | 现状复用平台 `/platform/sla-management` |
| 27 | 域运营概览 | `/domain/overview` | 已建 | F3.10 域运营概览【部分·P1】 | §7 F3.10 行 | §8 F3.10 行（domain.overview.read） | 统计「—」、趋势「数据接入中」 |
| 28 | 域级操作日志/登录日志 | `/domain/settings/audit-logs`、`/domain/settings/login-logs` | 已建 | F3.12 域级操作日志/登录日志【已实现·P0】 | §7 F3.12 行 | §8 F3.12 行（domain.audit_log.read / domain.login_log.read） | 审计不可删除 |
| — | （目标态移除） | `/system/user`、`/system/dept` | — | F3.15 系统用户管理、F3.16 系统部门管理【占位（模板遗留）】 | — | — | 2026-08-12 决策：域端业务由 F3.3 成员管理覆盖，模板遗留页移除 |

#### 9.2.3 管理端-平台端（M4）

| 序号 | 页面/模块 | 路由 | 页面状态 | 承载功能明细（名称【状态·优先级】） | 操作（§7 引用） | 权限码（§8 引用） | 备注/关键边界 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 29 | 登录页 | `/login` | 已建 | F4.5 平台端登录与动态菜单【已实现·P0】 | §7 F4.5 行 | 无码（登录公开） | 平台/业务域共用；忘记密码占位 stub |
| 30 | 平台首页仪表盘 | `/platform/home` | 已建 | F4.6 平台首页仪表盘【部分·P0】 | §7 F4.6 行 | §8 F4.6 行（platform.dashboard.read） | DemoDataService mock，未接真实聚合 |
| 31 | 用户管理 | `/platform/user` | 已建 | F4.7 用户管理【已实现·P0】；F4.3 员工账号与离职池【部分·P0】；**F4.18 用户导入导出【规划中（占位）·P1】并入** | §7 F4.7 行；§7 F4.3 行；§7 F4.18 行 | §8 F4.7 行（platform.user.*）；§8 F4.3 行（platform.user.offboard_pool.*）；§8 F4.18 行（platform.user.import） | 目标态：跨域批量停用入口（step-up 二次认证，TR-04）；F4.7 永久删除无前端入口；F4.3 CSV 导入未实现 |
| 32 | 离职池 | `/platform/offboard-pool` | 已建 | F4.3（引用：离职池子功能） | §7 F4.3 行 | §8 F4.3 行 | 离职池含域名/角色/离职时间/操作人 |
| 33 | 组织/部门管理 | `/platform/dept` | 已建 | F4.8 组织/部门管理【已实现·P0】；**F4.21 组织配置【规划中（占位）·P1】并入** | §7 F4.8 行；§7 F4.21 行 | §8 F4.8 行（platform.organization.*）；§8 F4.21 行 | 树形 CRUD + 循环引用检测；目标态合并 F4.21 |
| 34 | 角色管理 | `/platform/role` | 已建 | F4.9 角色管理【已实现·P0】 | §7 F4.9 行 | §8 F4.9 行（platform.role.* / platform.role_permission.* / platform.role.bind） | 目标态：新增「模板」Tab（role_template apply/同步/漂移） |
| 35 | 权限管理入口 | `/platform/permission` | 已建（重定向） | F4.11 权限管理入口【部分·P1】 | §7 F4.11 行 | 复用 platform.role.* | 仅 `<Navigate to="/platform/role">` |
| 36 | 菜单管理 | `/platform/system/menu` | 已建 | F4.10 菜单管理【已实现·P0】 | §7 F4.10 行 | §8 F4.10 行（platform.menu.*） | 菜单/按钮节点标签 |
| 37 | 业务域管理（列表/详情 10 Tab） | `/platform/domains`、`/platform/domains/detail/:domainId` | 已建 | F4.1 业务域创建与管理【已实现·P0】；F4.19 域配置 KV【已实现·P0】；F4.20 客户入域邀请码面板【部分·P0】 | §7 F4.1 行；§7 F4.19 行；§7 F4.20 行 | §8 F4.1 行（platform.domain.*）；§8 F4.19 行（domain.config.* 复用）；§8 F4.20 行（domain.invitation_code.*） | 详情 10 Tab 同源承载 F3.1/F3.3/F3.5/F3.9/F3.12/F3.13/F4.19/F4.20 |
| 38 | 事项类型配置/表单设计器/属性插槽（域详情内） | `/platform/domains/ticket-type-config/*`、`/platform/domains/ticket/form-design/*`、`/platform/domains/ticket-type-attributes/*` | 已建 | F3.1（引用）；F3.4（引用） | §7 F3.1 行；§7 F3.4 行 | §8 F3.1 行；§8 F3.4 行 | 与域端 `/domain/ticket-config` 同源 |
| 39 | 事项配置 | `/platform/ticket-config/*` | 已建 | F4.14 事项配置（类型/属性/状态/模板）【已实现·P0】；F4.2 模板中心【部分·P0】沿用 | §7 F4.14 行；§7 F4.2 行 | §8 F4.14 行（platform.ticket_config.* 16 码）；§8 F4.2 行（platform.ticket_config.template.*） | F4.2 目标态沿用 templates 子页；「域模板提炼」语义待对齐；菜单已隐藏 |
| 40 | SLA 管理 | `/platform/sla-management` | 已建 | F4.15 SLA 规则与工作日历【已实现（未挂验收 Story）·P0】；F3.2（引用：平台级保留） | §7 F4.15 行；§7 F3.2 行 | §8 F4.15 行（domain.sla.* 平台侧复用）；§8 F3.2 行 | 计时引擎 SlaTimingEngine 已实现；域级 SLA 目标态迁域端 |
| 41 | 审计/登录日志统一页 | `/platform/audit-logs` | 已建 | F4.12 审计日志/登录日志（平台统一页）【已实现·P0】 | §7 F4.12 行 | §8 F4.12 行（platform.log.audit.read / platform.log.login.read） | 独立页 `/platform/log/*` 双入口，目标态收敛 |
| 42 | 全局屏蔽词 | `/platform/blockwords` | 已建 | F4.13 全局屏蔽词【已实现·P0】 | §7 F4.13 行 | §8 F4.13 行（platform.blocked_word.*） | 跨域生效（business_domain_id 为空） |
| 43 | 站内信 | `/platform/inbox` | 已建 | F4.16 站内信（管理端）【已实现·P0】 | §7 F4.16 行 | §8 F4.16 行（inbox.read / inbox.mark_read） | — |
| 44 | 系统设置 | `/platform/system-settings` | 已建 | F4.4 系统设置与安全告警【部分·P0】 | §7 F4.4 行 | §8 F4.4 行（platform.system_config.*） | 密码强度/登录锁定/IP 白名单未成品 |
| 45 | 安全告警中心 | `/platform/security-alerts`（建议） | **目标态新增·页面未建** | F4.4（引用：安全告警部分） | §7 F4.4 行 | §8 F4.4 行 | 2026-08-12 决策：独立页 |
| 46 | 附件上传 | `/platform/attachments` | 已建 | F4.17 附件上传（MinIO）【已实现·P0】 | §7 F4.17 行 | §8 F4.17 行（attachment.upload / attachment.download） | 依赖 MinIO 服务（外部依赖 ADR） |
| 47 | 域配置 KV | `/platform/domain-config` | 已建 | F4.19（引用：独立页） | §7 F4.19 行 | §8 F4.19 行 | 与域详情「配置」Tab 同源 |
| 48 | 客户入域邀请码面板 | `/platform/domain-onboarding` | 已建 | F4.20（引用：独立页） | §7 F4.20 行 | §8 F4.20 行 | 与域详情「客户入域」Tab 同源（待确认） |
| 49 | 模板遗留页 | `/access/*`、`/route-nest/*`、`/about`、`/outside/*` 等 | 已建占位 | F4.22 模板遗留页面【占位（模板遗留）·P2】 | §7 F4.22 行 | 无码（模板遗留） | 目标态：异常页/公开页保留，demo 页移除 |
