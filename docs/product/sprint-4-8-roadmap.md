...
| 5 | `ticket-dynamic-form`（已有任务） | **已取消（2026-08-13 用户决策）**：管理端事项配置已实现，客户端渲染不在当前范围；F1.1 动态字段待后续另行立项 | — | — | — |
...
| # | 任务（slug） | 内容 | 优先级 | 依赖 | 验收要点 |
|:---|:---|:---|:---|:---|:---|
| 18 | `prd-doc-fixes`（已有任务） | **已取消（2026-08-13 用户决策）**：文档维护并入各 Sprint 执行时顺手进行 | — | — | — |
