-- 业务域端「工作台」菜单固化：/home + domain.home.read + domain_admin/agent 授权
...
WHERE NOT EXISTS (
    SELECT 1 FROM iam_admin_menu WHERE code = 'BUSINESS-HOME-MENU'
)
AND NOT EXISTS (
...
SELECT 'BUSINESS-HOME-BTN-READ', 'button', 'business', '查看业务域首页', 'domain.home.read',
       ...
       0, 1, 1
WHERE NOT EXISTS (
    SELECT 1 FROM iam_admin_menu WHERE code = 'BUSINESS-HOME-BTN-READ'
)
AND NOT EXISTS (
...
WHERE r.scope = 'domain'
  ...
  AND (
      ...
      OR m.code = 'BUSINESS-HOME-MENU'
      OR m.code = 'BUSINESS-HOME-BTN-READ'
