...
class DomainCustomerControllerTests {
    ...
    @Test
    void getCustomerReturnsView() throws Exception {
        MockMvc mockMvc = mockMvc();
        when(domainCustomerService.getCustomer(10L, 1L)).thenReturn(customerView(1L, "active"));

        mockMvc.perform(get("/api/v1/admin/domains/10/customers/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.status").value("active"));
        verify(domainCustomerService).getCustomer(10L, 1L);
    }
