...
6. **临时性 / 测试性文件落盘约束**  
   - **唯一允许目录**：Agent 因调试、探测、抓取日志、一次性脚本、冒烟验证等需要创建的临时性文件与测试性文件，**仅允许**写入仓库根目录下的 `agent-work/`（该目录已在 `.gitignore` 中忽略）。  
   - **禁止散落**：未经用户明确允许，**不得**在 `UnionDesk/`、`UnionDeskWeb/`、项目根目录及其他业务/源码目录创建此类文件（含 `_tmp_*`、`*-test-output.txt`、临时截图、本地探测脚本等）。  
   - **正式测试除外**：纳入工程的正式单元测试 / 集成测试（如 `src/test/`、`*.test.ts(x)`）按既有目录与规范编写，不受本条「临时目录」限制。  
   - **用完即清**：任务结束后应清理本次在 `agent-work/` 中产生、且不再需要的临时文件；不得把密钥、真实密码等敏感信息写入可被提交的路径。
